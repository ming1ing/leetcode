...
#### 解决方法:
>最先能想到的就是O(n^2)的方法,每一个位置找到第一个左边比它小和第一个右边比它小的位置,这样len=r[i]-l[i]-1,更新res=max(len*height[i],res),但是复杂度有点高,所以需要用单调栈来降低复杂度。用一个栈记录一个单调递增的序列，栈只放index，当遇到小于栈顶的元素，就把栈顶元素出栈；或遇到栈为空，把当前index入栈
