...
#### 解决方法：
>直接模拟，用4个变量记住边界，l,r,top,bot,先top的左往右，top--;
>r的上往下,r--;bot的右往左,bot--;l的下往上,l++;top>bot或者l>r的时候退出。
### 61.旋转链表
#### 题意：
...
#### 解决方法:
>对于/../返回上一层，对于/./保持现在位置,/../../..无限上一层最终还是/
### 82 删除排序链表中的重复元素 II
#### 题意:
>给定一个排序链表，删除所有含有重复数字的节点，只保留原始链表中 没有重复出现 的数字。
#### 解决方法:
>用三个指针，先使ppre=NULL,phead=head,pnext=head->next,如果phead和pnext有NULL,返回phead.判断phead->val和pnext->val是否相等，不相等时，再判断是否相邻，相邻就把phead放在ppre的链表里面。
>指针跳到下一个不相等元素:phead=pnext，pnext=pnext->next，再循环，注意对边界的处理.
### 83 删除排序链表中的重复元素
#### 题意:
...
>注意你不能在买入股票前卖出股票。
#### 解决方法:
>对于每一个买入点找它最大的卖出点。更新`ans=max(ans,r[i]-prices[i])`
### 122  买卖股票的最佳时机II
#### 题意:
>给定一个数组，它的第 i 个元素是一支给定股票第 i 天的价格。  
...
#### 解决方法:
>尺取，寻找序列的所有递增连续子序列，用ans加上连续子序列的尾和头的差值

### 141 环形链表
#### 题意:
>给定一个链表，判断链表中是否有环。
#### 解决方法:
>使用快慢指针，快指针一次移动2个元素，慢指针移动1个元素，当快指针和慢指针指向同一个元素为true,如果指向NULL为false。
### 145二叉树的中序遍历
#### 题意：
>给定一个二叉树，返回它的 中序 遍历。
#### 解决方法：
...
#### 解决方法:
>首先求出两条链表的长度，如果不相等，就把长的链表前面的一段去掉，使得两条链表长度相等，遍历两条长度相等的链表，当他们的值相等时，true，如果整条链不存在相等时，false
### 172 阶乘后的零
#### 题意：
...
#### 解决方法:
>先判断头节点是否为空，是否等于val`if(head==NULL) return NULL;head->val=val,head=head->next`
...
>请判断一个链表是否为回文链表。
#### 解决方法:
>先用快慢指针找到中点，分成两条链，把后面的链反转，判断反转后的链和前链是否相等，（如果原来的链表是奇数，只需要判断前面的元素，多出来的不用判断）
### 237.删除链表中的节点
#### 题意:
>请编写一个函数，使其可以删除某个链表中给定的（非末尾）节点，你将只被给定要求被删除的节点。
#### 解决方法:
>把链表node节点开始的每一个节点的后一个节点的值赋给前一个节点，删除最后一个节点
...
#### 解决方法:
>用三个指针，一个指向odd，一个指向even，一个指向even的next
>代码如下：`       `
...
#### 解决方法:
>使用sum记录，比当前位置的值要大的所有元素的和，使用后序遍历，把sum加上当前的值，并把当前值变为sum。
### 617. 合并二叉树
#### 题意:
>给定两个二叉树，想象当你将它们中的一个覆盖到另一个上时，两个二叉树的一些节点便会重叠。
你需要将他们合并为一个新的二叉树。合并的规则是如果两个节点重叠，那么将他们的值相加作为节点合并后的新值，否则不为 NULL 的节点将直接作为新二叉树的节点。
#### 解决方法:
>判断节点是或否为空，如果其中一个空就返回，另外一个非空节点。如果都非空，就new 一个节点，val等于他们的val和，再返回，最后递归调用左右子树.
### 697 数组的度
#### 题意
...
#### 解决方法:
>先定义一个Listnode节点，其他操作以这个节点进行。注意边界条件的判断和操作
### 876链表的中间结点
#### 题意：
